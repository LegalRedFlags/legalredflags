...
export default function CaseDetail({ c }: { c: CaseStudy }) {
  ...
  const title = isZh ? c.title : c.titleEn;
  const tags = isZh ? c.tags : c.tagsEn;
  const summary = isZh ? c.summary : c.summaryEn;
  const timeline = isZh ? c.timeline : c.timelineEn;
  const redFlags = isZh ? c.redFlags : c.redFlagsEn;
  const lessons = isZh ? c.lessons : c.lessonsEn;
    ...
    <>
      ...
      <article className="max-w-[960px] mx-auto px-7 pt-16 pb-20">
        ...
        <section className="mb-10">
          <h2 className="font-serif text-[18px] font-bold mb-3">{t("案件概述", "Case Summary")}</h2>
        ...
        <section className="mb-10">
          <h2 className="font-serif text-[18px] font-bold mb-5">{t("事件经过", "Timeline")}</h2>
        ...
        <section className="mb-10">
          <h2 className="font-serif text-[18px] font-bold mb-3">{t("涉及的危险信号", "Red Flags Involved")}</h2>
        ...
        <section className="mb-10">
          <h2 className="font-serif text-[18px] font-bold mb-3">{t("经验教训", "Lessons Learned")}</h2>
          <div className="border border-[var(--rule)]">
        ...
        <div className="bg-[var(--ground-warm)] border border-[var(--rule)] p-6 text-[13px] text-[var(--grey)] leading-relaxed">
          <strong className="text-[var(--ink-soft)]">{t("声明：", "Disclaimer: ")}</strong>
          {t(
            "以上案例已经过严格的匿名化处理，所有个人信息、公司名称、具体金额和地点均已调整或替换，以保护当事人隐私。案例的核心事实和欺诈模式是真实的。",
            "This case has been strictly anonymized. All personal information, company names, specific amounts, and locations have been adjusted or replaced to protect privacy. The core facts and fraud patterns are real."
          )}
        </div>
        ...
        <div className="mt-10 bg-[var(--surface)] border border-[var(--rule)] p-8 max-sm:p-6 text-center">
          <h3 className="font-serif text-[18px] font-bold mb-2">
            {t("关于这个案例，你怎么看？", "What do you think about this case?")}
          </h3>
          <p className="text-[14px] text-[var(--grey)] mb-5 max-w-[440px] mx-auto">
            {t(
              "有类似经历？有不同看法？欢迎在讨论区分享你的想法。",
              "Had a similar experience? A different perspective? Share your thoughts in the discussion."
            )}
          </p>
          <a
            href={`https://github.com/LegalRedFlags/legalredflags/discussions/new?category=general&title=${encodeURIComponent(`${isZh ? "案例" : "Case"} #${c.num}${isZh ? "：" : ": "}${title}`)}`}
            target="_blank"
        ...
        <div className="mt-8 pt-8 border-t border-[var(--rule)]">
          <Link href="/cases" className="text-sm text-[var(--green)] no-underline hover:underline">
            &larr; {t("返回全部案例", "Back to all cases")}
          </Link>
        </div>
      </article>
      <Footer />
